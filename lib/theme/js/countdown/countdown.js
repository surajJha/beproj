...
(function() {
    ...
    c.fixHexColor = function(a) {
        ...
        if (a.length < 1 || a == "#")
            return null;
        for (a.substr(0, 1) != "#" && (a = "#" + a); a.length < 7; )
            a += "0";
        return a
    };
    ...
    c.getXYWH = function(a) {
        var b = 0, c = 0, e = 0, g = 0;
        if (a) {
            for (var e = a.offsetWidth, g = a.offsetHeight, f = jbeeb.Browser.touch; a && !isNaN(a.offsetLeft) && !isNaN(a.offsetTop); )
                f ? (b += (a.offsetLeft || 0) - (a.scrollLeft || 0), c += (a.offsetTop || 0) - (a.scrollTop || 0)) : (b += a.offsetLeft || 0, c += a.offsetTop || 0), a = a.offsetParent;
                        ...
                        if (document.readyState != "loaded" && document.readyState != "complete")
                            ...
                            if (d === void 0) {
                                for (var a = document.getElementsByTagName("link"), b = 0; b < a.length; b++)
                                    a[b].getAttribute("rel") == "stylesheet" && d++;
...
(function() {
    ...
    c.bV = function() {
        ...
        for (c.ad.unshift(c.aU() - a); c.ad.length > 100; )
            c.ad.pop();
        for (c.L.unshift(a); c.L.length > 100; )
            c.L.pop()
    };
...
(function() {
    ...
    if (a.ie && a.version > 8)
        a.modern = true;
    if (a.chrome || a.safari || a.ios || a.android)
        a.modern = true;
...
(function() {
    ...
    b.init = function(a) {
        ...
        if (!a.editable)
            b.userSelect = b.WebkitUserSelect = b.MozUserSelect = b.OUserSelect = "none", b.MozUserSelect = "-moz-none";
        this.setCursor("inherit");
        if (a.center)
            this.autoCenter = a.center;
    ...
    b.applySkin = function(a, b) {
        ...
        if (a.image)
            c = a.image, typeof c == "string" ? (d = c, c = null) : (d = c.url, c = c.mode), this.setImage(d, c);
    ...
    b.ax = function() {
        ...
        if (a) {
            ...
            if (f) {
                ...
                m = m.join(",");
                if (f.modern)
                    b = f.cssPrefix, l = (h == "v" ? "top, " : "left, ") + m, b = b == "webkit" ? f.oldWebkit ? "-webkit-gradient(" + (h == "v" ? "linear, left top, left bottom,  " : "linear, left top, right top,  ") + r.join("\t,") + ") " : "-webkit-linear-gradient(" + l + ") " : b == "moz" ? "-moz-linear-gradient(" + l + ") " : b == "o" ? "-o-linear-gradient(" + l + ") " : b == "ms" ? "-ms-linear-gradient(" + l + ") " : "linear-gradient(" + ((h == "v" ? "to bottom, " : "to right, ") + m) + ") ";
                else if (jbeeb.Browser.ie)
                    h = "progid:DXImageTransform.Microsoft.gradient( GradientType=" + (h == "v" ? "0" : "1") + ", startColorstr='" + l[0] + "', endColorstr='" + l[l.length - 3] + "')", this.style.filter = h, this.style.msFilter = '"' + h + '"'
            }
            a.backgroundColor = c || "";
            a.backgroundImage = b;
            a.backgroundSize = d;
    ...
    b.stretch = function(a, b) {
        ...
        if (this.O) {
            if (this.O == "r" && this.parent) {
                if (this.aj == null)
                    this.aj = this.parent.width - this.x;
        ...
        if (this.V) {
            if (this.V == "b" && this.parent) {
                if (this.aO == null)
                    this.aO = this.parent.height - this.y;
                c = this.parent.height - this.aO;
                this.setY(c)
            }
        } else
            this.originY ? (e = this.originY, this.setY(e + (d - e) * b)) : this.setY(d * b)
    };
    b.aj = null;
    b.aO = null;
    b.setPin = function(a) {
    ...
    b.setFlex = function(a) {
        ...
        this.aA = this.bL = 0;
        if (a)
            a = a.toLowerCase(), this.bL = a.match(/w/) ? 1 : 0, this.aA = a.match(/h/) ? 1 : 0
    };
    ...
    b.bB = function() {
        ...
        if (b == [] && c == [] && d == [] && e == [])
            ...
            for (var b = c.concat(d, e, b), c = b.length, d = [], e = [], g = 0, f = 0; f < c; f++)
                g == 0 ? b[f] == 1 && e.push("inset") : g < 4 ? e.push(b[f] + "px") : (e.push(jbeeb.Utils.makeColor(b[f], b[f + 1])), d.push(e.join(" ")), e = [], ++f, g = -1), g++;
    ...
    b.addMEL = function(a, b, c, d) {
        this.MELbubble = c;
        if (!this.bu)
            this.bu = new jbeeb.MouseEventListener(this);
...
(function() {
    ...
    b.init = function(a) {
        if (a) {
            ...
            if (a.shadow)
                this.shadowText = a.shadow;
            if (a.inset)
                this.insetText = a.inset;
            if (a.bevel)
                this.bevelText = a.bevel;
            if (a.outline)
                this.outlineText = a.outline;
    ...
    b.G = function() {
        if (this.text != "") {
            var a, b = this.style;
            if (this.textSize)
                a = this.textSize, b.lineHeight = "1em";
            else {
                var c = this.width, d = this.height;
                if (c > 0 && d > 0)
                    if (this.textFit == "wh")
                        a = a = c < d ? c : d, a = this.textScale > 0 ? a * this.textScale : a;
                    else if (this.textFit == "w") {
                        if (c = this.width / this.measureText().w / 2, jbeeb.Utils.isNumber(c) && c > 0)
                            this.textScale = c, a = d * c
                    } else
                        a = d * this.textScale;
                else
                    a = 0;
    ...
    b.ab = function() {
        ...
        if (b == [] && c == [] && d == [] && e == [])
            ...
            for (var b = c.concat(d, b, e), c = b.length, d = [], e = [], g = 0, f = 0; f < c; f++)
                g == 0 ? b[f] == 1 && e.push("inset") : g < 4 ? e.push(b[f] + "px") : (e.push(jbeeb.Utils.makeColor(b[f], b[f + 1])), d.push(e.join(" ")), e = [], ++f, g = -1), g++;
...
(function() {
    ...
    b.init = function(a) {
        this.bo(a);
        if (a)
            this.stage = this.amStage == 1 ? this : this.stage, this.K = []
    };
    b.bN = b.stretch;
    b.stretch = function(a, b) {
        for (var c = this.K.length; c--; )
            this.K[c].stretch(a, b);
        this.bN(a, b)
    };
    b.be = b.destroy;
    b.destroy = function() {
...
var CountdownImageFolder = "images/", CountdownImageBasename = "flipper", CountdownImageExt = "png", CountdownImagePhysicalWidth = 41, CountdownImagePhysicalHeight = 60, CountdownWidth = 200, CountdownHeight = 30;
(function() {
    var c = function(a) {
    ...
    b.init = function() {
        ...
        if (a.time)
            this.J.setTime((b.getTime() / 1E3 + parseInt(a.time) + 1) * 1E3);
        else {
            ...
            d.getHours();
            if (b > d.getHours())
                b = this.J.getTime(), b += 864E5, this.J = new Date(b)
        }
        ...
        for (d = 0; d < b.length; d++)
            e = b[d], e == f && (f = d), e == h && (h = d);
        for (d = 0; d < b.length; d++)
            if (d >= f && d <= h)
                this.bq[b[d]].use = true;
        ...
        if (a.numbers)
            for (var n in p)
                a.numbers[n] && (p[n] = a.numbers[n]);
        if (a.labels)
            for (n in w)
                a.labels[n] && (w[n] = a.labels[n]);
        b.reverse();
        this.aN = {};
        for (d = n = 0; d < b.length; d++) {
            ...
            if (this.bq[k].use) {
                ...
                if (this.T == "flip") {
                    ...
                    k.time = new jbeeb.TextBox({x: 0, y: 0, w: m, h: r, text: "00", textScale: e, font: p.font, textColor: p.color, align: "center"}), k.addChild(k.time), k.line = new jbeeb.Box({x: 0, y: 0, w: m, h: g * 0.03, fill: "#000000"}), k.addChild(k.line), k.line.center();
                q = b[d].toUpperCase() + "S";
